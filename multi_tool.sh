...
install() {
	if [ "$port" = "9001" ]; then
		...
		. <(wget -qO- https://github.com/MrN1x0n/MrN1x0n/raw/main/insert_variable.sh) -n "minima_node_info" -v ". <(wget -qO- https://github.com/MrN1x0n/Minima/raw/main/node_info.sh) -l RU 2> /dev/null" -a
	else
		local node_name="minima_${port}"
		. <(wget -qO- https://github.com/MrN1x0n/MrN1x0n/raw/main/insert_variable.sh) -n "${node_name}_node_info" -v ". <(wget -qO- https://github.com/MrN1x0n/Minima/raw/main/node_info.sh) -l RU -p $((port+1)) 2> /dev/null" -a
	fi
